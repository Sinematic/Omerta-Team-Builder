...
export default function LadderElement({ player, ladderType } : { player: playerLadderDataType, ladderType: LadderType }) {
    ...
    const tearDrop = 
        <svg viewBox="0 0 31 39" fill="none" xmlns="http://www.w3.org/2000/svg">
            <ellipse cx="15.3272" cy="14.5378" rx="15.3272" ry="14.5378" fill="#202020"/>
            <path d="M2.55407 22.5215L15.4239 13.346L28.0997 22.5216L16.6045 38.787L2.55407 22.5215Z" fill="#202020"/>
    ...
    return (
        <li className={clsx(
            "w-[465px] h-[39px] relative flex place-items-center font-semibold border-b border-solid border-[rgb(var(--lightest-gray))] text-sm align-middle pl-2 pr-3 text-white",
            player.rank as number <= 3 ? "bg-[#121212]" : "bg-[#212121]/50"
        )}>
            <p className={clsx(
                "font-bold w-[65px] text-lg flex-shrink-0",
                player.rank === 1 ? "text-[#FFAA11]" : "", player.rank === 2 ? "text-[#7A7A7A]" : "", player.rank === 3 ? "text-[#754200]" : "")}>
                {player.rank as number > 9 ? player.rank  as number : ("0" + (player.rank))}
            </p>

            <p className="w-[122px] flex-shrink-0">{player.name}</p>

            <p className="w-[135px] flex align-middle flex-shrink-0 justify-center">
                ...
                : player.perc &&
                    <span style={{ color: colors.golden }} className="h-[20px] w-[26px] flex-shrink-0 flex items-center">
                        {percIcon}
                    </span>
                }
            ...
            {player.allowedHotSpot && <p className="w-[15px] absolute top-2.5 right-33">{tearDrop}</p> }

            {ladderType ===  "Ladder Classique" && <p className="pl-5 w-[20px]">{player.score}</p>}
            {ladderType ===  "Ladder Focus" && <p className="pr-8 w-[180px]">{player.score}</p>}

            <p className={clsx( "w-[100%] text-end pr-2", lineThrough )}>
        ...
        // <li className={clsx(
        //     "w-[465px] h-[39px] relative flex place-items-center font-semibold space-between border-b border-solid border-[rgb(var(--lightest-gray))] text-sm align-middle pl-2 pr-3 text-white",
        //     player.rank as number <= 3 ? "bg-[#121212]" : "bg-[#212121]/50"
        // )}>
        //     <p className={clsx( "font-bold w-[65px] text-lg flex-shrink-0",
        //         player.rank === 1 ? "text-[#FFAA11]" : "", player.rank === 2 ? "text-[#7A7A7A]" : "", player.rank === 3 ? "text-[#754200]" : "")}
        //     >
        //         {player.rank as number > 9 ? player.rank  as number : ("0" + (player.rank))}
        //     </p>

        //     <p className="w-[110px] flex-shrink-0">{player.name}</p>

        //     <p className="w-[135px] flex align-middle flex-shrink-0 justify-center">

        //         {player.perc &&
        //             <span style={{ color: colors.golden }} className="h-[22px] w-[26px] flex-shrink-0 flex items-center">
        //                 {percIcon}
        //             </span>
        //         }
        ...
        //     <p className={clsx( "w-fit text-end pr-2", lineThrough )}>
        //         {new Intl.NumberFormat("fr-FR").format(player.share)}
        //     </p>
