...
export default function SelectPlayers({ participants, handleClickOnParticipants, players } : SelectPlayersProps) {
    ...
    return (<>
        ...
        <ul className="grid grid-cols-3 gap-2 max-w-xl mx-auto select-none">
            {players.map((player) => 
                <li className={`px-4 py-2 text-center cursor-pointer ${participants.includes(player.name) ? " bg-green-600 text-white" : "bg-white"}`} 
